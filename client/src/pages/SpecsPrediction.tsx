...
export default function SpecsPrediction() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      // Ambil data langsung dari response object, bukan merujuk pada array 'data'
      const responseData = res.data;

      if (responseData.recommended_specs) {
        setResult(responseData.recommended_specs);
      } else {
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <header className="text-center">
        ...
        <p className="mt-2 text-gray-600 dark:text-gray-400">
          Pilih budget dan lokasi untuk melihat spesifikasi rumah yang ideal.
        </p>
      </header>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFilters({ ...filters, range_harga: e.target.value })
              }
              className="w-full rounded-xl border border-gray-300 bg-white/50 p-3 outline-none transition-all focus:ring-4 focus:ring-indigo-500/20 dark:border-gray-700 dark:bg-gray-800/50"
            >
              {options.range_harga_list.map((r) => (
        ...
        <div className="mx-auto max-w-4xl animate-in zoom-in duration-500">
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            <SpecCard
              ...
              label="Kamar Tidur"
              value={result.kamar_tidur}
            />
            <SpecCard
              icon={<Bath className="text-cyan-500" />}
              label="Kamar Mandi"
              value={result.kamar_mandi}
            />
            <SpecCard
              icon={<Maximize className="text-emerald-500" />}
              label="L. Tanah"
              value={`${result.luas_tanah} m²`}
            />
            <SpecCard
              icon={<Maximize className="text-green-500" />}
              label="L. Bangunan"
              value={`${result.luas_bangunan} m²`}
            />
            <SpecCard
              icon={<Zap className="text-yellow-500" />}
              label="Listrik"
              value={`${result.daya_listrik} W`}
            />
            <SpecCard
              icon={<Layers className="text-purple-500" />}
              label="Lantai"
              value={result.jumlah_lantai}
            />
            <SpecCard
              icon={<Car className="text-orange-500" />}
              label="Carport"
              value={result.carport}
            />
            ...
            <div className="flex flex-col items-center justify-center rounded-2xl border border-indigo-500/20 bg-indigo-500/5 p-6 text-center backdrop-blur-md dark:bg-indigo-500/10">
              ...
              <p className="text-xs font-semibold leading-tight">
                Spesifikasi paling umum di range ini.
              </p>
            </div>
          </div>
        </div>
